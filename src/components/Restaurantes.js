...
export default function Restaurantes() {
    const paperStyle={padding:'10px 10px', width:350, margin:"10px auto"}
    ...
    React.useEffect(()=>{
        ...
        .then(res => res.json())
        .then((result) =>{
            let filtRest = result.filter(res => res.categoria_id==="2");
            result = filtRest;
    ...
    <Container>
        ...
        <Paper elevation={3} style={paperStyle}>
            {restaurantes.map(restaurante => (
                ...
                <Paper elevation={6} style={{margin:"10px", padding:"15px", textAlign:"left"}} key={restaurante.id} >
